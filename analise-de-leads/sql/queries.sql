...
select distinct professional_status -- verificar quais sao as profissoes q tem na coluna professional_status
	...
	faixa_de_idade_dos_veiculos as(
		...
		select
			...
			case
				...
				when(extract('year' from visit_page_date) - pro.model_year::int)  <= 4 then 'de 2 anos à 4 anos'
				when(extract('year' from visit_page_date) - pro.model_year::int)  <= 6 then 'de 4 anos à 6 anos'
				when(extract('year' from visit_page_date) - pro.model_year::int)  <= 8 then 'de 6 anos à 8 anos'
				when(extract('year' from visit_page_date) - pro.model_year::int)  <= 10 then 'de 8 anos à 10 anos'
...
select
	"idade do veículo",
	count(*)::float/(select count(*) from sales.funnel) as "Veículos visitados (%)", ordem
